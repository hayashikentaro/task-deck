...
TaskDeck launches the App Server through the configured `codex-app-server` agent profile command and communicates over stdio. The built-in `Codex App Server` profile starts `ai-agent-sandbox-agent-1` and runs `codex app-server --listen stdio://` inside it with `docker exec -i`, not `-it`, because App Server uses JSON over ordinary stdin/stdout pipes rather than a terminal.
