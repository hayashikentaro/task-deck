...
- keeps server-owned App Server and process observations out of UI-only inference;
...
- App Server status/request events are process observations, not product identity.
- Adapter-specific behavior should stay isolated enough that App Server tuning does not accidentally rewrite unrelated layers.
