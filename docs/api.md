...
`PATCH /api/tasks/:taskId/attention/acknowledge` clears the current attention event for a running task without stopping or modifying its active runtime. The task stores `attentionAcknowledgedAt`, returns to Not now by setting `attentionState` to `none`, and can surface again when future prompt, App Server request, or quiet detection sets a new attention state.

`DELETE /api/tasks` bulk-clears non-running tasks and their logs while preserving active tasks. `DELETE /api/tasks/:taskId` clears a single task; clearing an individual running task stops its active App Server runtime or PTY and removes that task.
