...
`GET /api/agent-sessions` returns saved Codex sessions used by Codex PTY fallback resume flows. They are derived from stored task metadata and Codex's container-side session JSONL storage under `/home/dev/.codex/sessions`.
