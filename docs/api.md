...
Tasks started with the built-in `taskdeck-manager` agent profile are global manager sessions. The server ignores any client-selected project cwd for that profile, launches it from the TaskDeck control root, marks the task with `isManager`, and injects the manager inbox/readable environment variables documented in `docs/taskdeck-actor-protocol.md`.
