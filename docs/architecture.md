...
`.taskdeck/` may contain sensitive task metadata, logs, session labels, attachments, received decision results, pending decision leases, local task card display order, and agent output. Do not commit or share it. `task-order.json` stores only the UI card order and does not change task state or App Server routing. `taskdeck-instance.json` stores the stable local `taskdeckInstanceId` used when TaskDeck initiates Decision Gateway phone pairing, sends manual or dynamic decision request source metadata, and polls the Decision Result Mailbox; Decision Gateway owns the paired mobile browser session and uses the instance id for mailbox return routing. `decision-gateway-mailbox.json` stores received decision results. `decision-gateway-leases.json` stores outbound decision leases with pending, received, delivered, failed, stale, or legacy status and is TaskDeck's local safety boundary for decision delivery paths.
...
Task input is sent through the fixed bottom composer. For Codex App Server tasks it becomes structured turn input. Uploaded attachments remain pending until the matching composer input is processed for delivery, then move into the task-owned attachment directory and become part of persisted task metadata. Images are delivered as native App Server `localImage` items; supported text, source, PDF, and Office files are delivered as local file-path context. The text composer remains editable so operators can draft while a task is busy, waiting for attention, disconnected, locked, or otherwise not currently sendable; the send and attachment controls reflect whether input can be delivered to the selected running task. It supports multi-line instructions. Enter sends, Shift+Enter inserts a newline, Cmd/Ctrl+Enter sends, and IME composition is preserved for Japanese input.
...
`codexAppServer.loginMethod` can be set in the same config files. The default is `deviceCode`, which sends `account/login/start` with `type: "chatgptDeviceCode"` and preserves the existing verification URL plus user-code flow. Set it to `browserRedirect` to send `type: "chatgpt"` and show the returned ChatGPT login URL in the task log. Later config sources override earlier ones, and invalid values are ignored so the default device-code behavior remains available.

After App Server authentication is ready, TaskDeck requests `model/list` and broadcasts the normalized catalog to connected web clients. The selected task's composer uses that catalog for model and reasoning-effort controls. Each submitted instruction carries its selection atomically; the server records it on the task and passes `model` and `effort` to `turn/start`, so changes apply to that turn and subsequent turns without restarting the shared runtime or thread. Inputs queued before thread readiness retain the selection made when they were submitted. While a Codex App Server turn is active, the composer primary action becomes a stop control that asks the server to send `turn/interrupt` for that active `threadId` and `turnId`.
