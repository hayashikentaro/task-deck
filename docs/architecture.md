...
`codexAppServer.loginMethod` can be set in the same config files. The default is `deviceCode`, which sends `account/login/start` with `type: "chatgptDeviceCode"` and preserves the existing verification URL plus user-code flow. Set it to `browserRedirect` to send `type: "chatgpt"` and show the returned ChatGPT login URL in the task log. The web composer also shows a direct login action while a login is in progress. Later config sources override earlier ones, and invalid values are ignored so the default device-code behavior remains available.
