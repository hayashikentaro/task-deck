...
Codex App Server launches through `docker exec -i ... codex --sandbox danger-full-access app-server --listen stdio://` so TaskDeck can communicate over ordinary stdin/stdout pipes. The committed profile uses `danger-full-access` inside the configured Docker container to avoid nested Codex sandbox setup there; TaskDeck does not otherwise synthesize Codex CLI/TUI reasoning, startup, or resume flags for this profile. Prefer machine-readable or non-TUI agent modes when an agent exposes one; keep the PTY path for shell and non-Codex provider compatibility.
