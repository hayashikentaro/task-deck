...
Project suggestions come from `projectRoot` or `projectRoots` in `taskdeck.config.json`, `taskdeck.local.json`, `TASKDECK_PROJECT_ROOT`, or `TASKDECK_PROJECT_ROOTS`. The committed default uses `/Users/hayashikentarou/Documents`. TaskDeck lists immediate child directories under configured roots, skips hidden/build/cache directories, prefers git repositories, and falls back to the repository root when no project root is configured.
